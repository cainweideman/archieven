...
if data:
    ...
    for index, page in enumerate(text_list):
        ...
        for i in page_lines:
            name, rest = split_by_name(i)
            print(repr(i))  # Print the line for debugging

            if name and rest:
                prefix, remaining = find_prefixes(rest[0])
                if prefix:
                    name.append(prefix)  # Append prefix to the name
                    rest = remaining  # Update the remaining part of the text

            rest = "".join(rest).strip(',').strip(' ')  # Clean up the rest of the text
            print(f'name: {name}\nrest: {repr(rest)}\n')
