...
def filter_address(person):
	...
	if isinstance(address, list):
		return find_item_with_digit(address)
...
if data:
	...
	for index, page in tqdm(enumerate(text_list), total=len(text_list), ncols=100, unit='page'):
		...
		complete_lines = [line.strip() for line in complete_lines if len(line.split(',')) > 1]
		for line in complete_lines:
			...
			person['address'] = filter_address(person)
			person['jobTitle'] = filter_job(person)
			#print(person)
			#print('\n')
			if person['address'] == "None" or person['jobTitle'] == "None":
