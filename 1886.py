...
# Main execution
path_to_json = 'data/1886/text/1886.json'
data = open_json(path_to_json)
json_list = []

if data:
	text = get_text(data, first_page=44, last_page=191)
	...
	for line in text_list:
		...
		name_list, rest_list = split_on_initials(sentence)
		job, address = split_job_and_street(' '.join(rest_list))
		prepositions = extract_prepositions(line)
		name_list = name_list + prepositions
		if len(name_list) > 1 and len(address) >= 1:
			complete_name = format_name(name_list)
			person = make_json_object(complete_name, ' '.join(job).strip(), ' '.join(address).strip().rstrip('.').strip())
